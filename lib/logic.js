...
async function setupDemo(setupDemo){
    ...
    for(var i = 0; i<3; i++) {
        ...
        campaign.campaignDescription = 'This is the description for this campaign.'; 
        ...
        for (var j=0; j<2; j++) {
          ...
          const activity = factory.newResource(NS,'Activity', String(counter));
          counter++; 
          activity.activityName = 'Activity ' + j;
          activity.activityDescription = 'This is the description for this activity.';
          activity.bonusEducoin = 2;
          activity.maxStudents = 3;
          activity.currentStudents = 0;
          activity.campaign = campaign; 
          activity.dueDate = new Date(2018, 10, 31, 12, 0, 0, 0);
         
          var totalEducoin = 0;
          activity.assignments = [];
          for(k=0; k<2; k++){
              var assignment = factory.newConcept(NS,'Assignment');
              assignment.name = "Assignment " + k; 
              assignment.description = "This is the description for this assignment."; 
              assignment.educoin = 3; 
              activity.assignments[k] = assignment; 
              totalEducoin += assignment['educoin']; 
          }
          activity.educoin = totalEducoin; 
          await activityRegistry.add(activity);
        }
        campaign.fundingGoal = 40; 
...
/**
 * When a student submit an activity
 * @param {org.bfore.StudentSubmitActivity} StudentSubmitActivity - the  StudentSubmitActivity transaction
 * @transaction
 */
async function StudentSubmitActivity(studentSubmitActivity){
  const NS = 'org.bfore';
  if(studentSubmitActivity.reviewDate == null) {
    const activityContractRegistry = await getAssetRegistry(NS + '.ActivityContract');
    var today = new Date();
    studentSubmitActivity.activityContract.attachments = []; 
    studentSubmitActivity.activityContract.submissionDate = today; 
    for(var i = 0 ; i < studentSubmitActivity.attachments.length ; i++ ){
      studentSubmitActivity.activityContract.attachments[i] = studentSubmitActivity.attachments[i]; 
    }

    // Update activity Contract registry 
    await activityContractRegistry.update(studentSubmitActivity.activityContract);
  }
}
...
async function StudentCompleteActivity(studentCompleteActivity){
    ...
    const NS = 'org.bfore';
    if(studentCompleteActivity.activityContract.completed == true)
      return ;
    ...
    if(studentCompleteActivity.activityContract.dueDate.getTime() 
        >= studentCompleteActivity.activityContract.submissionDate.getTime()){
        ...
        // so that students cannot complete this activity twice
        studentCompleteActivity.activityContract.completed = true; 
        var today = new Date(); 
        studentCompleteActivity.activityContract.reviewDate = today; 
...
async function  EnrollStudentToActivity(enrollStudentToActivity){
    ...
    if(enrollStudentToActivity.activity.campaign.funded == true &&
        ...
        if (result.length == 0){
            const activityContractRegistry = await getAssetRegistry(NS + '.ActivityContract');
            let existingAssets = await activityContractRegistry.getAll();
            ...
            let activityContractId =  String(numberOfAssets +1); 
            const activityContract = factory.newResource(NS,'ActivityContract', activityContractId);
            activityContract.dueDate = enrollStudentToActivity.activity.dueDate;
            ...
            activityContract.completed = false; 
            // update ActivityContract registry
...
async function  addActivityToCampaign(addActivityToCampaign){
    ...
    if (currentParticipant.completed == false){
      // ActivityID creation
      const activityRegistry = await getAssetRegistry(NS + '.Activity');
      let existingAssets = await activityRegistry.getAll();
      ...
      let activityId =  String(numberOfAssets +1);  
        const factory = getFactory();
        const activity = factory.newResource(NS,'Activity', activityId);
        activity.activityName = addActivityToCampaign.activityName;
        activity.activityDescription = addActivityToCampaign.activityDescription;
        activity.bonusEducoin = addActivityToCampaign.bonusEducoin;
        activity.maxStudents = addActivityToCampaign.maxStudents;
        activity.dueDate = addActivityToCampaign.dueDate; 
        activity.currentStudents = 0;
        activity.assignments = []; 
        var totalEducoin = 0;
...
async function fundCampaign(fundCampaign){
    ...
    if (fundCampaign.campaign.funded == false && fundCampaign.campaign.completed == true && fundCampaign.educoinAmount > 0){
        ...
        //Check that the donor is the current participant
        if(fundCampaign.donor != currentParticipant)
          return; 
